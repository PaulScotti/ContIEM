...
% ou = unique(mydata2.condsX); 
% ou = ou(~isnan(ou));
% targ_chan = ceil(n_ori_chans/2);
% 
% chan_resp_shift = [];
% for ii = 1:size(chan_resp,1)
%     if ~isnan(mydata2.condsX(ii))
%         chan_resp_shift(end+1,:) = circshift(chan_resp(ii,:),targ_chan-find(ou==mydata2.condsX(ii)),2);
%     end
% end
...
% qq=mean(mean(chan_resp_shift));
% for o = 1:9
%     chan_resp_shift(:,o) = qq;
% end
...
save(strcat(home,num2str(SubjNum),'/Session_1/iem_plots/A1c_fit',ROI),'fit');
...
print(strcat(home,num2str(SubjNum),'/Session_1/iem_plots/A1c_',ROI),'-dpng');
end
