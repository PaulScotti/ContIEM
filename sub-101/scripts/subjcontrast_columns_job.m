...
if ~(7 == exist([root_dir 'cTrial40'],'dir')) %does Trial40 folder exist? if not, create the folders
    for i = 1:40 %make folder for every trial in a run (Mumford method)
        mkdir (['cTrial' num2str(i)])
    end
end
...
for trialnow = startTrial:numTrials
    for run = 1:numRuns
        ...
        for con = 1:11
            ...
            matlabbatch{1}.spm.stats.fmri_spec.sess(run).cond(con).duration = 0; %0 if event-related, otherwise time from onset to offset of stim
            matlabbatch{1}.spm.stats.fmri_spec.sess(run).cond(con).tmod = 0;
            matlabbatch{1}.spm.stats.fmri_spec.sess(run).cond(con).pmod = struct('name', {}, 'param', {}, 'poly', {});
            matlabbatch{1}.spm.stats.fmri_spec.sess(run).cond(con).orth = 1;
        end

        condition = 1;
        matlabbatch{1}.spm.stats.fmri_spec.sess(run).cond(condition).name = 'CurColor';
        matlabbatch{1}.spm.stats.fmri_spec.sess(run).cond(condition).onset = trial_onset_list{run}(trialnow);
        for trial = 1:numTrials
            if trial ~= trialnow
                ...
                matlabbatch{1}.spm.stats.fmri_spec.sess(run).cond(condition).onset = ...
                    [matlabbatch{1}.spm.stats.fmri_spec.sess(run).cond(condition).onset; trial_onset_list{run}(trial)];
            end
        end
    ...
    %Rename SPM.mat
    movefile('SPM.mat',['SPM_Trial_' num2str(trialnow) '.mat'])
    %Move SPM.mat to suitable folder
    movefile(['SPM_Trial_' num2str(trialnow) '.mat'],[root_dir '/cTrial' num2str(trialnow)])
    
    sprintf('Completed SPM for trial %i',trialnow)
end
